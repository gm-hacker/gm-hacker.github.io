...
通俗点说,在Java代码中,使用**JDBC提供的方法**,可以**发送**字符串类型的SQL语句到数据库管理软件(MySQL,Oracle等),并且获取语句执行结果!

进而实现数据库数据CURD操作的技术!

![image-20231219212147168](image/image-20231219212147168.png)

![image-20231219212206543](image/image-20231219212206543.png)

![image-20231219212304656](image/image-20231219212304656.png)
...
- 类加载知识补充
  ...
  类加载：就是将类的class文件读入内存，并创建一个`Java.lang.class`对象。类在运行期第一次使用时，首先会被类加载器动态加载至JVM。类的加载过程又分为了五个阶段：加载、验证、准备、解析、初始化。为了方便记忆可以编为一个口诀：家(加载)宴(验证)准备(准备)了西(解析)式(初始化)菜。
...
![img](image/image-17043743390351.png)
...
- 代码结构设计
  ...
  ![img](image/image-17043814419513.png)
...
【javax.sql.DataSource接口 规范了连接池获取连接的方法 规范了连接池回收连接的方法
...
![img](image/image-17043822563377.png)
...
- **项目介绍**
  ...
  ![img](image/image-17043824834699.png)

  修改客户

  ![img](image/image-170438249336411.png)

  展示客户列表

  ![img](image/image-170438250223413.png)

  删除客户

  ![img](image/image-170438251188615.png)

  退出系统

  ![img](image/image-170438252365417.png)

- **项目导入**

  1. 打开项目

     ![img](image/image-170438253479619.png)

  2. 配置jdk

     ![img](image/image-170438255119021.png)

     ![img](image/image-170438256221323.png)

### 7.2 基于cms项目添加数据库相关配置
