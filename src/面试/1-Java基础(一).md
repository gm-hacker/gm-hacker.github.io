...
```mermaid
flowchart LR
	A(.java) --> B(javac 编译) -->C(.class) --> D(解释器&JIT) --> E(机器可理解的代码) --> F(计算机) 
```
...
```mermaid
flowchart LR
	A(.java) --> B(javac 编译) -->C(.class) --> G{热点代码?}
	G -- YES --> D(JIT) --> E(计算机) 
	G -- NO --> H(解释器) --> E
```
...
```mermaid
flowchart TD
	A(编译型语言源代码) --> B(编译器) -- 一次性编译完成 -->C(机器可理解的代码) --> D(计算机) 
	E(解释型语言源代码) --> F(解释器) -- 一句一句解释 --> C
```
...
成员变量有默认值是为了确保在对象创建时，所有成员变量都有一个初始值，避免出现未初始化的情况。这样可以保证程序的可靠性和安全性。

当我们创建一个对象时，Java会自动为对象的成员变量分配内存空间，并根据数据类型给它们赋予默认值。这样做的好处有以下几点：

1. 避免未定义的行为：如果成员变量没有默认值，那么在对象创建后，它们将包含未知的、随机的数据。这可能导致程序的不确定行为，使得程序难以预测和调试。通过提供默认值，可以避免这种情况，确保成员变量的初始状态是可控的。
2. 方便使用：有了默认值，我们在创建对象后就可以直接使用成员变量，而不需要显式地进行初始化操作。这样可以简化代码，提高开发效率。
3. 兼容性考虑：Java是一门面向对象的编程语言，支持类的继承。在子类中，如果没有显式地对父类的成员变量进行初始化，那么它们将继承父类成员变量的默认值。这样可以确保子类对象在创建时，所有继承的成员变量都有一个合理的初始值。

综上所述，成员变量有默认值是为了确保程序的可靠性和安全性，避免未定义的行为，并方便开发者使用和维护代码。这是Java语言设计的一项重要特性。
...
发生在同一个类中（或者父类和子类之间），方法名必须相同，参数类型不同、数量不同、顺序不同，方法返回值和访问修饰符可以不同。
