...
mysql> SELECT last_name "Name", salary*12 "Annual Salary"
    -> FROM employees;
+-------------+---------------+
| Name        | Annual Salary |
+-------------+---------------+
...
mysql> SELECT DISTINCT department_id,salary
    -> FROM employees;
+---------------+----------+
...
mysql> SELECT employee_id, salary, commission_pct, 12 * salary * (1 + commission_pct) "annual_sal"
    -> FROM employees;
+-------------+----------+----------------+------------+
...
上面的sql在Windows都能正常执行，但是在Linux中，表名区分大小写，ORDER的查询语句执行错误，表不存在。
