...
![image-20231129141135520](image/image-20231129141135520.png)
...
![image-20231129141210627](image/image-20231129141210627.png)
...
+------+-------+-------------+--------------------+
|    1 |       | 13710011002 | 101223199012015623 |
|    2 |       | 13710011003 | 101223199012015624 |
+------+-------+-------------+--------------------+
...
![image-20231129145109717](image/image-20231129145109717.png)
...
![image-20231129153729555](image/image-20231129153729555.png)
...
+--------------------+-------------------+-----------------+----------------+------------+-----------------+
| def                | test01_libiary    | PRIMARY         | test01_libiary | emp        | PRIMARY KEY     |
| def                | test01_libiary    | emp_ibfk_1      | test01_libiary | emp        | FOREIGN KEY     |
+--------------------+-------------------+-----------------+----------------+------------+-----------------+
2 rows in set (0.02 sec)
...
+-------+------------+----------+--------------+-------------+-----------+-------------+----------+--------+------+------------+---------+---------------+
| emp   |          0 | PRIMARY  |            1 | eid         | A         |           0 |     NULL | NULL   |      | BTREE      |         |               |
| emp   |          1 | deptid   |            1 | deptid      | A         |           0 |     NULL | NULL   | YES  | BTREE      |         |               |
+-------+------------+----------+--------------+-------------+-----------+-------------+----------+--------+------+------------+---------+---------------+
2 rows in set (0.00 sec)
...
在MySQL中，默认AUTO_INCREMENT的初始值是1，每新增一条记录，字段值自动加1。设置自增属性AUTO_INCREMENT）的时候，还可以指定第一条插入记录的自增字段的值，这样新插入的记录的自增字段值从初始值开始递增，如在表中插入第一条记录，同时指定id值为5，则以后插入的记录的id值就会从6开始往上增加。添加主键约束时，往往需要设置字段自动增加属性。
